...
class ScenarioGenerator:
            ...
            for attempt in tenacity.Retrying(retry_error_callback=lambda x: eprint(x)):
                ...
                with attempt:
                    ...
                    if self.config['load_scenario_response']:
                        ...
                        self.num_humans = scenario_parsed['num_humans']
                    else:
                        #GENERATE SCENARIO
                        while True:
                            ...
                            self.num_humans = scenario.numberofhumans
                            expected_robot_behavior = scenario.expected_robot_behavior
                            behaviors_parsed = {}
                            ...
                            iprint(f'Expected robot behavior: {expected_robot_behavior}')
                            # if self.debug:
                            #     rprint("Reasoning:")
                            ...
                            if continue_choice:
                                ...
                                with open(self.config['scenario_file'],'w') as f:
                                    json.dump({'scenariodescription':scenario.scenariodescription,
                                               'humanbehavior':behaviors_parsed,
                                               'num_humans':scenario.numberofhumans},f)
                                break
                    ...
                    if self.config['load_trajectory_response']:
                        ...
                        while True:
                            #GENERATE TRAJECTORY
                            if edits == None:
                                trajectories = self.get_trajectories(scenario_desc,self.num_humans)
                            else:
                                trajectories = self.get_trajectories(scenario_desc,self.num_humans,edits=edits,prev_output=trajectories)
    ...
    def instantiate_simulator(self,file_paths,groupids,trajectories,behaviors_trees):
        ...
        shutil.copyfile(self.config['trajectory_file'], os.path.join(file_paths['hunav_gazebo_wrapper_dir'],'config','trajectory.json'))
